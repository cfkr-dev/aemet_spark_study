...
export IMAGE_NAME="plot-generation-localstack:v1.0"
...
check_file "$AUTO_PLOT_BUILD_SCRIPT" || { SCRIPT_FAILED=1; exit $SCRIPT_FAILED; }
...
if docker_exists "$AUTO_PLOT_IMAGE_NAME"; then
    ...
    if ! "$AUTO_PLOT_BUILD_SCRIPT" --build-only; then
