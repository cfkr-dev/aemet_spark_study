"""Double linear model definitions and configuration containers.

.. module:: App.Api.Models.double_linear_model

Holds classes to represent configuration for double-linear plots where two
Y axes are present. Each class exposes ``setup`` and ``validate`` to
populate and check user-provided configuration mappings.
"""

import re

from pathlib import Path

from App.Config.enumerations import *
from App.Utils.Storage.Core.storage import Storage
from App.Utils.validator import Validator
from App.Utils.file_utils import get_src_path, get_dest_path


class AxisComponent:
    """Axis component with optional formatter enum.

    :ivar name: Component name.
    :type name: str
    :ivar format: Resolved formatter enum or None.
    :type format: object | None
    """

    def __init__(self):
        """Create an :class:`AxisComponent` with default empty values.

        Attributes are initialized to empty/None and should be populated
        using :meth:`setup`.
        """
        self.name = ""
        self.format = None

    def setup(self, data: dict):
        """Populate component from mapping.

        :param data: Mapping which may contain a ``'format'`` key.
        :type data: dict
        """
        self.name = data['name']
        self.format = get_enum_value(data.get('format'), Formatters)

    def validate(self, validator: Validator):
        """No-op validation for the axis component.

        :param validator: Validator to collect errors.
        :type validator: App.Utils.validator.Validator
        """
        return


class Axis:
    """Container for X and two Y axes (y_1, y_2).

    :ivar x: X axis component.
    :type x: AxisComponent
    :ivar y_1: First Y axis component.
    :type y_1: AxisComponent
    :ivar y_2: Second Y axis component.
    :type y_2: AxisComponent
    """

    def __init__(self):
        """Initialize the :class:`Axis` container with empty components."""
        self.x = AxisComponent()
        self.y_1 = AxisComponent()
        self.y_2 = AxisComponent()

    def setup(self, data: dict):
        """Populate axis components from mapping.

        :param data: Mapping with keys ``'x'``, ``'y_1'`` and ``'y_2'``.
        :type data: dict
        """
        self.x.setup(data['x'])
        self.y_1.setup(data['y_1'])
        self.y_2.setup(data['y_2'])

    def validate(self, validator: Validator):
        """Validate all contained axis components.

        :param validator: Validator to collect issues.
        :type validator: App.Utils.validator.Validator
        """
        self.x.validate(validator)
        self.y_1.validate(validator)
        self.y_2.validate(validator)


class Src:
    """Source configuration including path and axis mapping."""

    def __init__(self):
        """Create an empty :class:`Src` with default path and axis."""
        self.path: Optional[Path] = None
        self.axis = Axis()

    def setup(self, data: dict):
        """Populate source path and axis mapping from configuration.

        :param data: Mapping containing ``'path'`` and ``'axis'``.
        :type data: dict
        """
        self.path = get_src_path(data['path'])
        self.axis.setup(data['axis'])

    def validate(self, validator: Validator):
        """Validate that the source exists and axis mapping is correct.

        :param validator: Validator used to collect issues.
        :type validator: App.Utils.validator.Validator
        """
        if not validator.storage.exists(self.path.as_posix()):
            validator.set_invalid()
            validator.add_error_msg('Source path must be a valid path')

        self.axis.validate(validator)


class Dest:
    """Destination configuration for output files.

    :ivar path: Resolved destination directory.
    :type path: pathlib.Path | None
    :ivar filename: Output filename.
    :type filename: str
    :ivar export_png: Whether to export PNG.
    :type export_png: bool
    """

    def __init__(self):
        """Initialize :class:`Dest` with default empty values."""
        self.path: Optional[Path] = None
        self.filename = ""
        self.export_png = True

    def setup(self, data: dict):
        """Populate destination settings from mapping.

        :param data: Mapping with keys ``'path'``, ``'filename'`` and ``'export_png'``.
        :type data: dict
        """
        self.path = get_dest_path(data['path'])
        self.filename = data['filename']
        self.export_png = data['export_png']

    def validate(self, validator: Validator):
        """No-op destination validation for API compatibility."""
        return


class Lettering:
    """Textual labels used in the double-linear plot.

    :ivar title: Chart title.
    :type title: str
    :ivar subtitle: Optional subtitle.
    :type subtitle: str | None
    :ivar x_label: X axis label.
    :type x_label: str
    :ivar y_1_label: First Y axis label.
    :type y_1_label: str
    :ivar y_2_label: Second Y axis label.
    :type y_2_label: str
    """

    def __init__(self):
        """Create default lettering values for the double-linear chart."""
        self.title = ""
        self.subtitle = None
        self.x_label = ""
        self.y_1_label = ""
        self.y_2_label = ""

    def setup(self, data: dict):
        """Populate lettering from mapping.

        :param data: Mapping with keys for title, subtitle, and axis labels.
        :type data: dict
        """
        self.title = data['title']
        self.subtitle = data.get('subtitle', None)
        self.x_label = data['x_label']
        self.y_1_label = data['y_1_label']
        self.y_2_label = data['y_2_label']

    def validate(self, validator: Validator):
        """No-op validation for lettering."""
        return


class Figure:
    """Visual definition for a single figure line.

    :ivar name: Figure identifier.
    :type name: str
    :ivar color: Hex color string.
    :type color: str
    """

    def __init__(self):
        """Create a :class:`Figure` with empty name and color."""
        self.name = ""
        self.color = ""

    def setup(self, data: dict):
        """Populate figure settings from mapping.

        :param data: Mapping with keys ``'name'`` and ``'color'``.
        :type data: dict
        """
        self.name = data['name']
        self.color = data['color']

    def validate(self, validator: Validator):
        """Validate color format for the figure.

        :param validator: Validator used to collect errors.
        :type validator: App.Utils.validator.Validator
        """
        if not bool(re.fullmatch(r'#([0-9a-fA-F]{3}|[0-9a-fA-F]{6})', self.color)):
            validator.set_invalid()
            validator.add_error_msg('Color must be a valid hex color')


class Margin:
    """Margins wrapper with default fallbacks."""

    def __init__(self):
        """Initialize the :class:`Margin` container with default values."""
        self.left = 0.0
        self.right = 0.0
        self.top = 0.0
        self.bottom = 0.0

    def setup(self, data: dict):
        """Populate margin values from mapping.

        :param data: Optional mapping containing keys ``'left'``, ``'right'``, ``'top'`` and ``'bottom'``.
        :type data: dict
        """
        self.left = data.get('left', 80.0)
        self.right = data.get('right', 80.0)
        self.top = data.get('top', 80.0)
        self.bottom = data.get('bottom', 80.0)

    def validate(self, validator: Validator):
        """No validation performed for margins (kept for API parity)."""
        return


class Legend:
    """Legend configuration container."""

    def __init__(self):
        """Create :class:`Legend` with default visibility and offset."""
        self.show_legend = True
        self.y_offset = 0.0

    def setup(self, data: dict):
        """Populate legend settings from mapping.

        :param data: Mapping that may contain key ``'y_offset'``.
        :type data: dict
        """
        self.show_legend = True if data.get('y_offset') else False
        self.y_offset = data.get('y_offset', -0.3)

    def validate(self, validator: Validator):
        """No-op validation for legend."""
        return


class Style:
    """Container grouping lettering, plots, margins and legend."""

    def __init__(self):
        """Create :class:`Style` and initialize child components."""
        self.lettering = Lettering()
        self.figure_1 = Figure()
        self.figure_2 = Figure()
        self.margin = Margin()
        self.legend = Legend()

    def setup(self, data: dict):
        """Populate style configuration from mapping.

        :param data: Mapping with keys ``'lettering'``, ``'figure_1'``, ``'figure_2'`` and optional
                     ``'margin'``, ``'legend'``.
        :type data: dict
        """
        self.lettering.setup(data['lettering'])
        self.figure_1.setup(data['figure_1'])
        self.figure_2.setup(data['figure_2'])
        self.margin.setup(data.get('margin', {}))
        self.legend.setup(data.get('legend', {}))

    def validate(self, validator: Validator):
        """Validate the style subtree by delegating to children.

        :param validator: Validator used to collect errors.
        :type validator: App.Utils.validator.Validator
        """
        self.lettering.validate(validator)
        self.figure_1.validate(validator)
        self.figure_2.validate(validator)
        self.margin.validate(validator)
        self.legend.validate(validator)


class DoubleLinearModel:
    """Top-level model for double-linear chart configuration.

    Provides :meth:`setup` and :meth:`validate` used by controllers.
    """

    def __init__(self, storage: Storage):
        """Create a new DoubleLinearModel.

        :param storage: Storage backend used by validation routines.
        :type storage: App.Utils.Storage.Core.storage.Storage
        """
        self.storage = storage
        self.src = Src()
        self.dest = Dest()
        self.style = Style()

    def setup(self, data: dict):
        """Populate the model from a configuration mapping.

        :param data: Mapping containing ``'src'``, ``'dest'`` and ``'style'``.
        :type data: dict
        """
        self.src.setup(data['src'])
        self.dest.setup(data['dest'])
        self.style.setup(data['style'])

    def validate(self):
        """Validate the model and return a :class:`Validator` instance.

        :returns: Validator instance populated with validation results.
        :rtype: App.Utils.validator.Validator
        """
        validator = Validator(self.storage)

        self.src.validate(validator)

        self.dest.validate(validator)

        self.style.validate(validator)

        return validator
