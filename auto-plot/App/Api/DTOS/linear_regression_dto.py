"""
DTOs for Linear Regression endpoint.

Defines the request payload model (flask-restx) expected by the linear-regression API.

.. module:: App.Api.DTOS.linear_regression_dto
"""
...
class LinearRegressionDTO:
    """Container for the Linear Regression endpoint request model.

    Exposes `post_input` which is a flask-restx model describing the expected JSON
    payload for POST requests to the linear-regression endpoint.
...
def _create_input_post_dto(ns: Namespace):
    ...
    :type ns: flask_restx.Namespace
    :returns: A flask-restx model describing the linear-regression POST payload.
    """
